...
class KcBERTDataset(Dataset):
    def __init__(self, dataset, sent_idx=0, label_idx=1, max_len=64):
        self.dataset = dataset
        self.max_len = max_len
        self.tokenizer = BertTokenizer.from_pretrained("beomi/kcbert-large")
        self.sentences = [self.transform(i[0]) for i in self.dataset]
        self.fuck_label = [np.int32(i[1]) for i in self.dataset]
        self.badword_label = [np.int32(i[2]) for i in self.dataset]
    ...
    def __getitem__(self, idx):
        return self.sentences[idx] + (self.fuck_label[idx], ) + (self.badword_label[idx], )
